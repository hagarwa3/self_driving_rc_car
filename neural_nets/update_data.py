"""
This file serves to update our collected data so that it is stored in as noise free of a format as possible
"""
...
for single_npz in training_data:
	...
	with np.load(single_npz) as datas:
		...
		for data in datas['train']:
			...
			i5 = i4+1
			i+=1
			img1 = cv2.resize(cv2.imread('training_images/frame{:>05}.jpg'.format(i1),0), (0,0), fx=0.25, fy=0.25)
			img1 = draw_edges(img1, 0.05).flatten().astype(np.float32)
			img2 = cv2.resize(cv2.imread('training_images/frame{:>05}.jpg'.format(i2),0), (0,0), fx=0.25, fy=0.25)
			img2 = draw_edges(img2, 0.05).flatten().astype(np.float32)
			img3 = cv2.resize(cv2.imread('training_images/frame{:>05}.jpg'.format(i3),0), (0,0), fx=0.25, fy=0.25)
			img3 = draw_edges(img3, 0.05).flatten().astype(np.float32)
			img4 = cv2.resize(cv2.imread('training_images/frame{:>05}.jpg'.format(i4),0), (0,0), fx=0.25, fy=0.25)
			img4 = draw_edges(img4, 0.05).flatten().astype(np.float32)
			img5 = cv2.resize(cv2.imread('training_images/frame{:>05}.jpg'.format(i5),0), (0,0), fx=0.25, fy=0.25)
